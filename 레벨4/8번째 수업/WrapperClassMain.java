...
/*
래퍼 클래스 생성 - 박싱(boxing)
- 기본형을 래퍼 클래스로 변경하는 것을 마치 박스에 물건을 넣는것 가탇고 해서 박싱(Boxing)이라고 한다.
- new Integer(10) 은 사용하면 안된다. 현재까지 작동은 되지만, 향후 제거할 예정이다
...
- 추가로 Integer.valueOf() 에는 성능 최적화 기능이 있다.
	...
	비교는 equals() 사용
	- 래퍼 클래스는 객체이기 때문 == 비교를 하면 인스턴스이 참조값을 비교한다.
	- 래퍼 클래스는 내부 값을 비교하도록 equals()를 재정해두었다. 따라서 값을 비교하려면 equals()를 사용해야 한다.
	
	참고로 래퍼 클래스는 객체를 그대로 출력해도 내부에 잇는 값을 문자로 출력하도록 toString() 재정의했다.
