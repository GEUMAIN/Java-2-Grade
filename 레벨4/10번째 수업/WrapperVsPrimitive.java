...
/*
 ...
 래퍼 클래스의 인스턴스는 내부에 필드로 가지고 있는 기본형의 값 뿐만 아니라 자바에서 객체를 다루는데 필요한
 객체 메타데이터를 포함하므로 더 많은 메모리를 사용하게 된다. (대략 8~16바이트) 
 
 위 연산은 10억 번의 연산을 수행헸을 때 0.2동안 3.1초의 차이만큼 발생한다.
 기본형이든 래퍼 클래스든 이것을 1회로 화난하면 둘 다 매우 빠르게 연산이 수행된다.
 일반적으로 애플리케이션을 만드는 관점에서 보면 이런 부분을 최적화에도 사막의 모래알 하나 정도의 차이가 날뿐이다.
